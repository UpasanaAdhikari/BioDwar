<?php
    ...
    if(mysqli_num_rows($result)>0){
                $output='<table id="table_dataa" class="border-separate border-spacing-2 border border-slate-500">
                <tr>
                ...
                while($row=mysqli_fetch_assoc($result)){
                    ...
                    <td class='border bg-green-400'><button id='' data-etid='{$row['id']}' class='edit_btn_t cursor-pointer p-1' style='border:0;border-radius:5px;'>Edit</button></td>
                    <td class='border bg-red-400'><button id='' data-tid='{$row['id']}' class='delete_btn_t cursor-pointer p-1' style='border:0;border-radius:5px;'>Delete</button></td>
                    </tr>";
                }
            $output .="</table>";
            mysqli_close($conn);
